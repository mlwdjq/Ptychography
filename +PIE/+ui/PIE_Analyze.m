...
%                 defocus_pha = df_um.*real(kz);
            ...
            K = round(scanRange_um/this.do_um)+N;
            L = round(scanRange_um/this.do_um)+N; % size of object [K,L]
            if round(scanRange_um/this.do_um)<scanSteps
...
%             Rn = round(dPosShifts(:,2)*1000/do_um);
        ...
        function analyze(this)
            ...
            Rpix = round(Rprobe_um/this.do_um);
            if FP
            ...
            if sigma >0 %% using sigma filter remove noise
                ...
                selectedData(abs(selectedData-mean(selectedData))>sigma*rms)=0;
            ...
            for k = 1:length(dTabIdx)
                switch dTabIdx
                    ...
                    case  this.U8PROBEOBJECT
                        ...
                        xp_mm = [1:N]*this.do_um/1000; % object coordinates
                        Magnification = this.uieMag.get();
                        if this.uicbFourierPtychography.get()
                            object = PIE.utils.Propagate (this.dObject(:,:,u8ModeId),propagator,this.do_um,lambda_um,1);
                            xo_mm = ([1:L]-L/2)*this.dc_um*N/L/1000/Magnification; % object coordinates
                            yo_mm = ([1:K]-K/2)*this.dc_um*N/K/1000/Magnification; % object coordinates
                        else
                            object = this.dObject(:,:,u8ModeId);
                            xo_mm = ([1:L]-L/2)*this.do_um/1000; % object coordinates
                            yo_mm = ([1:K]-K/2)*this.do_um/1000; % object coordinates
                        end
                    ...
                    case this.U8ANALYSIS
                        ...
                        elseif strcmp(selectedObject,'Residual errors') ==1
                            ...
                            [K,L] = size(object);
                            x_mm = this.dUnit_mm*([1:L]-L/2);
                            y_mm = this.dUnit_mm*([1:K]-K/2);
