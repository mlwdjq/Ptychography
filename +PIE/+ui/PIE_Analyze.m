        ...
        function generateProbeObject(this)
            ...
            switch probeType
                case 'Defocus wave'
                    ...
                    n1 = n1-N/2-1;
                    n2 = n2-N/2-1;
                    %                     probe = ifftshift(ifft2(ifftshift(pinhole(round(2*Rc_um/dc_um),N,N).*...
                    %                         exp(-1i*pi*abs(z_um)*dc_um^2/lambda_um/z_um^2*(n1.^2+n2.^2)))));
                    s= exp(-1i*pi*abs(z_um)*dc_um^2/lambda_um/z_um^2*(n1.^2+n2.^2));
                    probe = PIE.utils.Propagate (pinhole(round(2*Rc_um/dc_um),N,N).*s,'fresnel',...
...
%                             title('object amplitude');drawnow;pause(1)
                    ...
                    case 'RAAR' % batch scanning solution
                        if i==1 % initial exitWaves
                            ...
                            for j =1:scanSteps^2
                                ...
                                exitWaves(:,:,j) = pthis.dProbeRecon.*reconBox;
                            end
                        end
                        parfor j =1:scanSteps^2
                            ...
                            exitWaves(:,:,j) = delta*(exitWaves(:,:,j) + exitWaveNew) +(1-2*delta)*this.dProbeRecon.*reconBox;
                        end
                    ...
                    case 'WDD' % Wigner distribution deconvolution
                        ...
                        for m = 1:scanSteps
                            for n = 1:scanSteps
                                temp1 = fftshift(fft2(fftshift(this.dObjectRecon(Rypix(m,n)+[1:N],Rxpix(m,n)+[1:N])))); % from R to U
                                temp0 = fftshift(fft2(fftshift(this.dObjectRecon(Rypix(round((scanSteps+1)/2),...
