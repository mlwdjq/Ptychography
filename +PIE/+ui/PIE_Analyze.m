classdef PIE_Analyze < mic.Base
    ...
    methods
        ...
        function generateProbeObject(this)
            ...
            dR = scanRange_um/(scanSteps-1); % scanning interval
            detSize_um  = this.uieDetSize.get()*1000;
            propagator = this.uipPropagator.getOptions{this.uipPropagator.getSelectedIndex()};
            dc_um       = detSize_um/N; % detector pixel pitch
            ...
            if isempty(probeOffset)
                ...
                probeOffset=round(probeOffset*1000./do_um);
            end
            ...
            if ~FP
                switch probeType
                    case 'Defocus wave'
                        ...
                        CO = 1-pinhole(round(2*Rc_um/dc_um*CenterObstruction),N,N);
                        probe = PIE.utils.Propagate (pinhole(round(2*Rc_um/dc_um),N,N).*CO.*Hs.*exp(1i*probe_pha),propagator,...
            ...
            if lComputePSStack % True if simulating "stack"
                ...
                for m = 1:length(dXPosShifts)
                    for k = 1:length(dYPosShifts)
                        %
                        Wint = PIE.utils.simulateDiffractionPattern(this.dProbe,this.dObject,this.ceSegments,modeNumber,N,...
                ...
                tic
                %
                Wint = PIE.utils.simulateDiffractionPattern(this.dProbe,this.dObject,...
                    this.ceSegments,modeNumber,N,propagator,[round(scanRange_um/do_um/2),round(scanRange_um/do_um/2)],H,1);
