...
%% dcFlare analysis
vals = linspace(0,0.1,11); % Transform from 64-px detector to 1k detector
nTrials = 1;
...
for m = 1:nTrials
    for k = 1:length(vals)
        ...
        val = vals(k);
        pie.setSimParams('flareLevel',val);
...
%% plot
figure,h=plot(vals(1:end)*100,mean(RMS,2));xlabel('DC flare / %'),ylabel('RMS object phase difference / rad');
set(h,'LineWidth',2); set(gca,'FontSize',14);

%% save data
save([pie.cAppPath,sprintf('/../../data/error analysis/dcFlare-study_%s.mat', regexprep(datestr(now, 31), ':', '.'))], 'vals', 'RMS');
