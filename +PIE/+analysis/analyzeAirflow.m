...
%% airflow analysis
vals = linspace(0,0.001,11); % Transform from 64-px detector to 1k detector
nTrials = 10;
...
%% plot
figure,h=plot(vals(1:end)*1000,mean(RMS,2));xlabel('Airflow / mWaves'),ylabel('RMS object phase difference / rad');
set(h,'LineWidth',2); set(gca,'FontSize',14);

%% save data
save([pie.cAppPath,sprintf('/../../data/error analysis/airflow-study_%s.mat', regexprep(datestr(now, 31), ':', '.'))], 'vals', 'RMS');
