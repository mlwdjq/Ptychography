...
%% s2s analysis
vals = linspace(0,10,11); % Transform from 64-px detector to 1k detector
nTrials = 10;
...
for m = 1:nTrials
    for k = 1:length(vals)
        ...
        val = vals(k);
        pie.setSimParams('shotToShot',val);
...
%% plot
figure,h=plot(vals(1:end),mean(abs(RMS),2));xlabel('Shot noise / %'),ylabel('RMS object phase difference / rad');
set(h,'LineWidth',2); set(gca,'FontSize',14);

%% save data
save([pie.cAppPath,sprintf('/../../data/error analysis/S2S-study_%s.mat', regexprep(datestr(now, 31), ':', '.'))], 'vals', 'RMS','object');
