function [ShearX,ShearY]=FindShearCoordinatesat(lambda,z1,z2,T,halfD,alpha,gamma)
x(1)=0;
i=1;
while 1
i=i+1;
th=atan(x(i-1)/(z1+z2));
s1       = z1 * sin(th)/cos(th - alpha);
ph=asin(sin(th-alpha)+lambda/T)+alpha;
if (z1 - s1 * sin(alpha))*tan(th)+(z2 - s1 * sin(alpha)+Z)*tan(ph)<=halfD
