...
function output = MHFPropC2(aperture, inCoordsX, inCoordsY, inCoordsZ, outCoordX, outCoordY, outCoordZ, wavelength,alpha)
...
% Check sampling:
lsianalyze.utils.HFSampling(aperture, inCoordsX, inCoordsY, inCoordsZ, outCoordX, outCoordY, outCoordZ, wavelength * 1000);

% Only pass nonzero components:
idx = find(aperture);
tic;
output = lsianalyze.utils.MHFPropCmex2(aperture(idx), inCoordsX(idx), inCoordsY(idx), inCoordsZ(idx), ...
                        outCoordX, outCoordY, outCoordZ, wavelength * 1000, alpha);
